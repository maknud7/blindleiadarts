...
repo = Path("apps/backend-v2/src/mysql/equipment-admin-repository.ts")
text = repo.read_text()
marker = '''  async ensureRuntimeAlias(environmentClubIdInput: unknown, physicalIdInput: unknown): Promise<string> {\n'''
method = '''  async deleteBoard(environmentClubIdInput: unknown, physicalIdInput: unknown): Promise<boolean> {
...
'''
if "async deleteBoard(" not in text:
    if marker not in text:
        raise SystemExit("ensureRuntimeAlias insertion point not found")
    text = text.replace(marker, method + marker, 1)

helper_marker = '''  private async generateKioskCodeWith(db: SqlExecutor, clubKey: string, boardNumber: number): Promise<string> {\n'''
helpers = '''  private async countBoardMatchesWith(db: SqlExecutor, environmentClubId: string, physicalId: string): Promise<number> {
...
'''
if "countBoardMatchesWith(" not in text:
    if helper_marker not in text:
        raise SystemExit("generateKioskCode insertion point not found")
    text = text.replace(helper_marker, helpers + helper_marker, 1)
repo.write_text(text)

router = Path("apps/backend-v2/src/runtime/equipment-admin-router.ts")
text = router.read_text()
router_marker = '''    const kiosk = /^\\/v1\\/clubs\\/([1-9][0-9]*)\\/kiosks\\/([1-9][0-9]*)$/.exec(path);
    if (kiosk && (method === "PATCH" || method === "PUT")) {
'''
router_replacement = '''    const kiosk = /^\\/v1\\/clubs\\/([1-9][0-9]*)\\/kiosks\\/([1-9][0-9]*)$/.exec(path);
    ...
    if (kiosk && (method === "PATCH" || method === "PUT")) {
'''
if 'kiosk && method === "DELETE"' not in text:
    if router_marker not in text:
        raise SystemExit("router kiosk insertion point not found")
    text = text.replace(router_marker, router_replacement, 1)
router.write_text(text)

proxy = Path("apps/api/src/BackendV2EquipmentProxyApplication.php")
text = proxy.read_text()
if "['PUT', 'PATCH', 'DELETE']" not in text:
    comment_old = "        // Physical board deletion remains on PHP until Node has identical match\n        // history and reference cleanup protection. Do not add DELETE here yet.\n"
    comment_new = "        // Physical board delete has the same history/reference safety contract in Node.\n        // TEST still fails closed before any canonical hardware mutation is attempted.\n"
    if comment_old not in text:
        raise SystemExit("proxy delete comment not found")
    text = text.replace(comment_old, comment_new, 1)
    start = text.index(comment_new)
    needle = "            return in_array($method, ['PUT', 'PATCH'], true);"
    pos = text.find(needle, start)
    if pos < 0:
        raise SystemExit("proxy kiosk method list not found")
    text = text[:pos] + "            return in_array($method, ['PUT', 'PATCH', 'DELETE'], true);" + text[pos + len(needle):]
proxy.write_text(text)

routing = Path("apps/api/tests/backend_v2_equipment_proxy_routing_test.php")
text = routing.read_text()
positive = "    ['DELETE', '/v1/clubs/1/kiosks/9'],\n"
if positive not in text:
    anchor = "    ['PATCH', '/v1/clubs/1/kiosks/9'],\n"
    if anchor not in text:
        raise SystemExit("routing PATCH anchor not found")
    text = text.replace(anchor, anchor + positive, 1)
negative = "// Irreversible physical delete stays on the PHP safety implementation until\n// Node has identical historical-reference checks and cleanup semantics.\n$assert(!$proxy->handles('DELETE', '/v1/clubs/1/kiosks/9'), 'Physical board delete must remain outside Node cutover.');\n"
text = text.replace(negative, "")
routing.write_text(text)

workflow = Path(".github/workflows/backend-v2-equipment-frontdoor.yml")
text = workflow.read_text()
old_grep = "          grep -Fq \"!\\$proxy->handles('DELETE', '/v1/clubs/1/kiosks/9')\" apps/api/tests/backend_v2_equipment_proxy_routing_test.php\n"
new_grep = "          grep -Fq \"['DELETE', '/v1/clubs/1/kiosks/9']\" apps/api/tests/backend_v2_equipment_proxy_routing_test.php\n"
if old_grep in text:
    text = text.replace(old_grep, new_grep, 1)
workflow.write_text(text)

scolia = Path("apps/backend-v2/src/mysql/scolia-admin-repository.ts")
text = scolia.read_text()
text = text.replace('\nthrow new DomainValidationError("bridge_attached_required"', '\n          throw new DomainValidationError("bridge_attached_required"')
text = text.replace("\n(kiosk_id,connection_state,board_status,board_phase,error_type,last_disconnect_reason,last_disconnect_at,connected_at)", "\n          (kiosk_id,connection_state,board_status,board_phase,error_type,last_disconnect_reason,last_disconnect_at,connected_at)")
text = text.replace("\n last_disconnect_reason=VALUES(last_disconnect_reason)", "\n           last_disconnect_reason=VALUES(last_disconnect_reason)")
text = text.replace("\n  SET status='expired'", "\n            SET status='expired'")
text = text.replace("\nWHERE kiosk_id=?", "\n          WHERE kiosk_id=?")
scolia.write_text(text)

test_file = Path("apps/backend-v2/test/equipment-delete-parity.test.mjs")
