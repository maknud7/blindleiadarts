...
async function loadLive() {
  ...
  try {
    state.live = await requestJson(url);
  } catch (error) {
    state.live = {
      tournament: null,
      feed: {
...
async function loadAdminFallback() {
  ...
  return {
    club,
    players: (playersData.items || []).map((player) => ({
      ...player,
      member_id: null,
      member_name: null,
      member_link_source: null,
      dartsatlas_external_id: null,
    })),
    members: [],
    tournaments: (tournamentsData.items || []).filter((item) => item.provider_system === "dartsatlas"),
    screens: screensData.items || [],
    dartsatlas: {
      season_external_id: "",
      tournament_count: (tournamentsData.items || []).filter((item) => item.provider_system === "dartsatlas").length,
      player_count: 0,
      poll_interval_seconds: 8,
    },
    member_registry: {
      available: false,
      source: "admin_endpoint_unavailable",
      member_count: 0,
      linked_player_count: 0,
      unlinked_player_count: (playersData.items || []).length,
    },
  };
}
...
async function loadAdmin() {
  try {
    ...
    state.adminEndpointAvailable = true;
  } catch (error) {
    state.adminEndpointAvailable = false;
    state.admin = await loadAdminFallback();
    state.admin.endpoint_error = error.message;
  }
}
...
async function loadAll() {
  ...
  try {
    await Promise.all([loadAdmin(), loadLive()]);
    renderAll();
    if (!state.adminEndpointAvailable) {
      showMessage(
        el.globalMessage,
        "Adminens DartsAtlas-endepunkt er ikke tilgjengelig på serveren ennå. Grunnfunksjonene virker, men medlemskobling er midlertidig deaktivert.",
        "warning",
      );
    }
  } catch (error) {
...
function renderMetrics() {
  ...
  el.metrics.innerHTML = [
    ...
    metric("Medlemskobling", registry.available ? `${registry.linked_player_count || 0}/${admin.players?.length || 0}` : "Ikke tilgjengelig", registry.available ? "Delt medlemsregister tilkoblet" : "Venter på admin-endepunkt", registry.available ? "good" : "neutral"),
...
function renderDartsAtlas() {
  ...
  el.dartsAtlasStatus.innerHTML = [
    ...
    ["Medlemskilde", feed.member_registry_source || admin.member_registry?.source || "ukjent", admin.member_registry?.available ? "good" : "neutral"],
    ["Bootstrap", bootstrap.status || "ikke kjørt", statusTone(bootstrap.status)],
  ...
  el.tournamentList.innerHTML = tournaments.map((tournament) => `
    <article class="list-row">
      <div><strong>${escapeHtml(tournament.name)}</strong><div class="row-meta"><span>${escapeHtml(formatDate(tournament.start_at))}</span><span>${escapeHtml(tournament.dartsatlas_external_id || "")}</span></div></div>
      <div class="row-right"><span class="badge ${statusTone(tournament.status)}">${escapeHtml(tournament.status)}</span><small>${Number(tournament.completed_match_count || 0)}/${Number(tournament.match_count || 0)} kamper ferdig</small></div>
    </article>`).join("");
}

function memberOptions(selectedId) {
  const members = state.admin?.members || [];
  return [`<option value="">— Ikke koblet —</option>`, ...members.map((member) => `<option value="${Number(member.id)}" ${Number(member.id) === Number(selectedId) ? "selected" : ""}>${escapeHtml(member.navn)} (#${Number(member.id)})</option>`)].join("");
}
...
function renderMembers() {
  ...
  el.memberRegistryStatus.innerHTML = registry.available
    ...
    : `<span class="dot neutral"></span><strong>Medlemskobling midlertidig utilgjengelig</strong><span>Resten av admin virker. Kobling aktiveres når DartsAtlas-admin-endepunktet er på plass.</span>`;
  ...
  el.playerRows.innerHTML = filtered.map((player) => {
    ...
    const linkLabel = player.member_id ? (player.member_link_source === "manual" ? "Manuell" : "Automatisk") : "Ikke koblet";
    const tone = player.member_id ? "good" : "warning";
    return `<tr>
      <td><strong>${escapeHtml(player.display_name)}</strong>${player.nickname ? `<small>${escapeHtml(player.nickname)}</small>` : ""}</td>
      <td><span class="badge neutral">${source}</span></td>
      <td><select class="member-select" data-player-id="${Number(player.id)}" ${registry.available ? "" : "disabled"}>${memberOptions(player.member_id)}</select></td>
      <td><span class="badge ${tone}">${escapeHtml(linkLabel)}</span>${player.member_name ? `<small>${escapeHtml(player.member_name)} · #${Number(player.member_id)}</small>` : ""}</td>
    </tr>`;
  }).join("");
  ...
  document.querySelectorAll(".member-select").forEach((select) => {
    select.addEventListener("change", async () => {
      ...
      try {
        ...
        showMessage(el.globalMessage, `Kunne ikke lagre medlemskobling: ${error.message}`, "error");
        await loadAdmin();
        renderAll();
      }
    });
...
function renderScreens() {
  ...
  el.screenList.innerHTML = screens.map((screen) => `
    <article class="list-row screen-row">
      <div><strong>${escapeHtml(screen.label || "Venue Screen")}</strong><div class="screen-code">${escapeHtml(screen.access_code || "")}</div><div class="row-meta"><span>Sist koblet: ${escapeHtml(formatDate(screen.last_connected_at))}</span></div></div>
      <div class="row-right"><span class="badge ${Number(screen.is_active) === 1 ? "good" : "bad"}">${Number(screen.is_active) === 1 ? "Aktiv" : "Inaktiv"}</span><button type="button" class="copy-button" data-code="${escapeHtml(screen.access_code || "")}">Kopier kode</button></div>
    </article>`).join("");
...
el.refreshDartsAtlasButton.addEventListener("click", () => loadAll());
