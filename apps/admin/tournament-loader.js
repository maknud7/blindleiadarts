...
const MODULE_VERSION = "20260920-admin-perf-01";
let requested = false;
let loading = null;
let waitTimer = null;
...
async function loadModules() {
  ...
  loading = (async () => {
    // Diagnostics is deliberately lightweight and observes the same requests
    // without owning the room lifecycle.
    await import(moduleUrl("./tournament-diagnostics.js"));

    // Use the proven all-at-once module order again. The recent lazy/deferred
    // bootstrap made unrelated admin sections and the tournament room depend on
    // timing between portal navigation, club context and dynamically injected scripts.
    ...
    await Promise.all([
      ...
      import(moduleUrl("./tournament-elo-admin.js")),
      import(moduleUrl("./admin-ux-bootstrap.js")),
    ]);
