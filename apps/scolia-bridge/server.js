...
async function spool(serial, message) {
  const record = {
...
async function flushSpool() {
  ...
  try {
    ...
    for (const name of files.slice(0, 200)) {
      ...
      try {
        await api("/scolia/bridge/events", { method: "POST", body: record });
...
class BoardConnection {
  ...
  async onOpen() {
    ...
    this.reconnectAttempt = 0;
    await spool(this.config.serial_number, internalMessage("BRIDGE_CONNECTED", { kiosk_id: this.config.kiosk_id }));
    flushSpool().catch(() => undefined);
    this.commandTimer = setInterval(() => this.pollCommands().catch((error) => console.warn(`Command poll board ${this.config.board_number}:`, error.message)), COMMAND_POLL_MS);
  }
  ...
  async onMessage(data) {
    ...
    await spool(this.config.serial_number, message);
    flushSpool().catch(() => undefined);
  }
  ...
  async onClose(code, reasonBuffer) {
    ...
    if (!this.closedByConfig) {
      await spool(this.config.serial_number, internalMessage("BRIDGE_DISCONNECTED", { code, reason }));
      flushSpool().catch(() => undefined);
  ...
  async onError(error) {
    this.state = "error";
    await spool(this.config.serial_number, internalMessage("BRIDGE_ERROR", { error: String(error?.message || error) }));
    flushSpool().catch(() => undefined);
  }
  ...
  async pollCommands() {
    ...
    const data = await api(`/scolia/bridge/commands/${this.config.kiosk_id}`);
    for (const command of data.items || []) {
      ...
      if (!this.send(outgoing)) {
        await api(`/scolia/bridge/commands/${command.id}/result`, { method: "POST", body: { result: "failed", error: "socket_not_open" } });
        continue;
      }
      const timeout = setTimeout(() => {
        this.pendingCommands.delete(command.message_id);
        api(`/scolia/bridge/commands/${command.id}/result`, {
          method: "POST",
...
async function reconcileConfig() {
  const data = await api("/scolia/bridge/config");
  const wanted = new Map((data.boards || []).map((board) => [Number(board.kiosk_id), board]));

  for (const [kioskId, connection] of connections.entries()) {
    const next = wanted.get(kioskId);
    if (!next) {
      connection.stop("board disabled");
      connections.delete(kioskId);
      continue;
    }
    const probe = new BoardConnection(next);
    if (connection.fingerprint() !== probe.fingerprint()) {
      connection.stop("configuration changed");
      const replacement = new BoardConnection(next);
      connections.set(kioskId, replacement);
      replacement.start();
    }
    wanted.delete(kioskId);
  }

  for (const [kioskId, board] of wanted.entries()) {
    const connection = new BoardConnection(board);
    connections.set(kioskId, connection);
    connection.start();
  }
}

async function heartbeat() {
  const boards = [...connections.entries()].map(([kioskId, connection]) => ({ kiosk_id: kioskId, state: connection.state }));
  await api("/scolia/bridge/heartbeat", { method: "POST", body: { boards } });
}

async function drainServerQueue() {
  await api("/scolia/bridge/drain", { method: "POST", body: { limit: 100 } });
}

async function boot() {
  console.log(`Blindleia Scolia Bridge starting. API=${API_BASE}, spool=${SPOOL_DIR}`);
