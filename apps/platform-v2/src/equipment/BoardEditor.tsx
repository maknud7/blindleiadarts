...
export function BoardEditor({ board, clubId, token, readOnly, onClose, onSaved }: Props) {
  ...
  async function pairTablet() {
    if (busy || board.is_paired || Number(board.is_active ?? 1) !== 1) return;
    const code = normalizePairingCode(pairCode);
    if (code.length < 4) {
      setError("Skriv inn koden som vises på nettbrettet.");
      return;
    }
    setBusy("pair");
    setError("");
    ...
    <section className="board-editor-card" role="dialog" aria-modal="true" aria-labelledby="board-editor-title">
      <div className="panel-head board-editor-head">
        <div>
          ...
          <p>Alt som gjelder denne skiva ligger her: scoring, Scolia og nettbrett.</p>
        </div>
        <button type="button" className="button secondary small" disabled={Boolean(busy)} onClick={onClose}>Lukk</button>
      </div>

      {readOnly && <div className="notice warn"><strong>TEST er skrivebeskyttet for fysisk oppsett.</strong> Pairing og TEST-runtime kan fortsatt brukes.</div>}
      ...
      <form onSubmit={submit}>
        ...
        <section className={`board-connection-card ${testTerminal ? "test-terminal" : ""}`}>
          ...
          <small>{board.is_paired ? `Paret ${formatDate(board.paired_at)} · sist sett ${formatDate(board.last_seen_at)}` : "Skriv inn koden som står på nettbrettet, eller scan QR-koden på nettbrettet med telefonen."}</small>
          {!board.is_paired && Number(board.is_active ?? 1) === 1 && <div className="board-pairing-entry"><label className="field"><span>Kode fra nettbrett</span><input value={pairCode} onChange={(event) => setPairCode(normalizePairingCode(event.target.value))} maxLength={12} autoComplete="off" autoCapitalize="characters" placeholder="B7K4P2" /></label><button type="button" className="button small" disabled={Boolean(busy) || pairCode.length < 4} onClick={() => void pairTablet()}>{busy === "pair" ? "Kobler …" : "Koble nettbrett"}</button></div>}
        </section>
