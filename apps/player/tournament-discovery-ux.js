...
style.textContent=`
  ...
  .tdx-detail-tabs{display:grid;grid-template-columns:repeat(4,1fr);gap:4px;background:#e9eff5;padding:4px;border-radius:14px;position:sticky;top:73px;z-index:1}
...
function renderDetail(t,players,matches,reg,tab){
  ...
  const adminTab=dialog.classList.contains("is-admin")?`<button class="tdx-admin-tab" type="button">Admin</button>`:"";
  dialog.innerHTML=`<div class="tdx-page"><div class="tdx-page-head"><button class="tdx-back">‹</button><div class="tdx-page-title"><h2>${esc(t.name)}</h2><p>${esc(fmtDate(t.start_at))}${fmtTime(t.start_at)?` · ${esc(fmtTime(t.start_at))}`:""}</p></div></div>
    ...
    <div class="tdx-detail-tabs"><button class="${tab==="overview"?"active":""}" data-tab="overview">Oversikt</button><button data-tab="groups">Grupper</button><button class="${tab==="players"?"active":""}" data-tab="players">Deltakere</button><button class="${tab==="matches"?"active":""}" data-tab="matches">Kamper</button>${adminTab}</div>${tab==="players"?playersHtml(players):tab==="matches"?matchesHtml(matches):overviewHtml(t,players,matches)}</div>${actionsHtml(t,reg)}</div>`;
