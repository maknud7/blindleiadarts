...
import type { MySqlTournamentAttendanceAdminRepository } from "../mysql/tournament-attendance-admin-repository.js";
import {
...
export interface TournamentAttendanceAdminRouteResult {
  statusCode: number;
  payload: Record<string, unknown>;
}

export class TournamentAttendanceAdminRouter {
  constructor(
    private readonly config: BackendRuntimeConfig,
    private readonly identityRepository: MySqlIdentityAuthRepository,
    private readonly attendance: MySqlTournamentAttendanceAdminRepository,
  ) {}

  async handle(method: string, path: string, request: IncomingMessage): Promise<TournamentAttendanceAdminRouteResult | null> {
    ...
    if (clubSettingsMatch && (method === "GET" || method === "PUT" || method === "PATCH")) {
      ...
      return ok({
        settings: await this.attendance.updateClubSettings(clubId, body, requiredId(user.id, "user_id")),
      });
    }
    ...
    if (tournamentSettingsMatch && (method === "GET" || method === "PUT" || method === "PATCH")) {
      ...
      assertMutationAllowed(this.config);
      return ok({ settings: await this.attendance.updateTournamentSettings(tournamentId, await readJsonObject(request)) });
    }
    ...
    if (method === "POST" && guestMatch) {
      ...
      const user = await this.requireUser(request);
      this.requireAdmin(user, requiredId(settings.club_id, "club_id"));
      return ok({ registration: await this.attendance.addGuest(tournamentId, await readJsonObject(request)) }, 201);
    }
  ...
  private requireAdmin(user: IdentityUser, clubId: string): void {
    ...
    if (role !== "club_admin") throw new RuntimeAccessError(403, "admin_required", "Admin-tilgang kreves.");
...
function requiredPlayerId(user: IdentityUser): string {
  ...
  if (playerId === null) {
    throw new DomainValidationError("player_profile_missing", "Kontoen er ikke koblet til en spillerprofil.");
  }
  return playerId;
}
...
function ok(payload: Record<string, unknown>, statusCode = 200): TournamentAttendanceAdminRouteResult {
