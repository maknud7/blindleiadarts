...
export interface TournamentOperationsRouteResult { statusCode: number; payload: Record<string, unknown>; }

export class TournamentOperationsLegacyRouter {
  constructor(
  ...
  async handle(method: string, path: string, request: IncomingMessage): Promise<TournamentOperationsRouteResult | null> {
    ...
    if (method === "POST" && moveMatch) {
      ...
      await this.requireAdmin(request, clubId);
      const body = await readJsonObject(request);
      const move = await this.operations.moveMatch(tournamentId, matchId, body.kiosk_id, body.confirm_in_progress === true);
  ...
  private identityTouchAllowed(): boolean {
    return ((this.config.environment === "prod" && this.config.prefixes.identity === "bd_prod_" && mutationsAllowed(this.config)) ||
...
async function readJsonObject(request: IncomingMessage): Promise<Record<string, unknown>> {
  ...
  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) throw new RuntimeAccessError(400, "invalid_json_object", "Request body must be a JSON object.");
  return parsed as Record<string, unknown>;
}
function ok(payload: Record<string, unknown>, statusCode = 200): TournamentOperationsRouteResult { return { statusCode, payload: { ok: true, ...payload } }; }
