...
export class MySqlScoliaKioskAuthRepository {
  ...
  /** Clear the device pairing after the same legacy-compatible access check used by scoring. */
  async unpairScoring(codeInput: unknown, tokenInput: unknown): Promise<Record<string, unknown>> {
    const kiosk = await this.resolveScoring(codeInput, tokenInput, false);
