...
final class BackendV2TournamentProxyApplication
{
    ...
    public function handles(string $method, string $path): bool
    {
        ...
        if (in_array($method, ['GET', 'PUT', 'PATCH'], true) && preg_match('#^/v1/tournaments/\d+/elo-settings$#', $path) === 1) return true;
        if (in_array($method, ['GET', 'PUT', 'PATCH'], true) && preg_match('#^/v1/tournaments/\d+/summary/admin$#', $path) === 1) return true;
        ...
        if ($method === 'GET' && preg_match('#^/v1/tournaments/\d+/check-in-status$#', $path) === 1) return true;
        if (in_array($method, ['GET', 'PUT', 'PATCH'], true) && preg_match('#^/v1/clubs/\d+/checkin-settings$#', $path) === 1) return true;
