...
CREATE TABLE IF NOT EXISTS `{{TABLE_PREFIX}}match_statistics` (
    ...
    CONSTRAINT `{{TABLE_PREFIX}}fk_match_statistics_match_id`
        FOREIGN KEY (`match_id`) REFERENCES `{{TABLE_PREFIX}}matches` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{TABLE_PREFIX}}fk_match_statistics_player_id`
...
CREATE TABLE IF NOT EXISTS `{{TABLE_PREFIX}}live_match_states` (
    ...
    CONSTRAINT `{{TABLE_PREFIX}}fk_live_match_states_match_id`
        FOREIGN KEY (`match_id`) REFERENCES `{{TABLE_PREFIX}}matches` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{TABLE_PREFIX}}fk_live_match_states_throwing_player_id`
        FOREIGN KEY (`throwing_player_id`) REFERENCES `{{TABLE_PREFIX}}players` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
