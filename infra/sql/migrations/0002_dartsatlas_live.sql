ALTER TABLE `{{TABLE_PREFIX}}players`
    ...
    ADD COLUMN `member_link_source` VARCHAR(50) DEFAULT NULL AFTER `member_id`,
    ADD COLUMN `member_linked_at` DATETIME DEFAULT NULL AFTER `member_link_source`,
    ADD KEY `idx_players_member_id` (`member_id`);

ALTER TABLE `{{TABLE_PREFIX}}matches`
    ADD COLUMN `provider_metadata` JSON DEFAULT NULL AFTER `bracket_label`;

CREATE TABLE IF NOT EXISTS `{{TABLE_PREFIX}}connector_resources` (
...
CREATE TABLE IF NOT EXISTS `{{TABLE_PREFIX}}match_statistics` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT `{{TABLE_PREFIX}}fk_match_statistics_match_id` FOREIGN KEY (`match_id`) REFERENCES `{{TABLE_PREFIX}}matches` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{TABLE_PREFIX}}fk_match_statistics_player_id` FOREIGN KEY (`player_id`) REFERENCES `{{TABLE_PREFIX}}players` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `{{TABLE_PREFIX}}live_match_states` (
    `match_id` BIGINT UNSIGNED NOT NULL,
    ...
    CONSTRAINT `{{TABLE_PREFIX}}fk_live_match_states_match_id` FOREIGN KEY (`match_id`) REFERENCES `{{TABLE_PREFIX}}matches` (`id`) ON DELETE CASCADE,
    CONSTRAINT `{{TABLE_PREFIX}}fk_live_match_states_throwing_player_id` FOREIGN KEY (`throwing_player_id`) REFERENCES `{{TABLE_PREFIX}}players` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
